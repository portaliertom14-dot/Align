/**
 * Écran questions Checkpoint 1 — 3 questions (Q1, Q2, Q3).
...
import { CHECKPOINT_1_QUESTIONS, SUBTITLE } from '../../data/checkpointQuestions';
import { saveCheckpointAnswer } from '../../lib/checkpointAnswers';

const CHECKPOINT_COLOR = '#FFD93F';
const QUESTIONS = CHECKPOINT_1_QUESTIONS;

export default function Checkpoint1QuestionScreen() {
  const navigation = useNavigation();
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [selectedAnswer, setSelectedAnswer] = useState(null);

  const currentQuestion = QUESTIONS[currentQuestionIndex];
  const progressPercent = ((currentQuestionIndex + 1) / 3) * 100; // 33, 66, 100
  const isLastQuestion = currentQuestionIndex === 2;

  const handleSelect = (option) => {
    setSelectedAnswer(option);
    saveCheckpointAnswer('checkpoint1', currentQuestionIndex, option);
    setTimeout(() => {
      if (isLastQuestion) {
        navigation.replace('Checkpoint2Intro');
      } else {
...
const styles = StyleSheet.create({
  ...
  optionButtonSelected: {
    ...
    backgroundColor: 'rgba(255, 217, 63, 0.15)',
  },
  optionText: {
    fontSize: 16,
    fontFamily: theme.fonts.button,
